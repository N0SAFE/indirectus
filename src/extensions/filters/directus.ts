...
export function has_system_collections(
...
export function has_non_system_collections(
    context: TemplateContext,
    collections: Collection[],
) {
    return collections.some((collection) => !collection.is_system);
...
export function filter_untype_system_collections(
    ...
    return collections.filter(
        (collection) => !["directus_versions"].includes(collection.name.raw),
    );
}

export function to_system_method_suffix(
    context: TemplateContext,
    ...
    return to_collection_name(context, collection.name.raw, true).replace(
        "Directus",
        "",
    );
}
    ...
    if (system) {
        if (partial) {
            name = regex_replace(
                ...
                pascal_case(
                    context,
                    drop_first(context, split(context, value, "_")),
                ),
                "s$",
                "",
            );
