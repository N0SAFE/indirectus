...
import { Generator, GeneratorOptions } from "@/types/generators/generator";
...
export default defineCommand({
    ...
    args: {
        url: {
            ...
            required: true,
            description:
                "The URL of the Directus instance to generate an SDK for",
    ...
    async run(context) {
        const options = await GeneratorOptions.get({
            ...
            output:
                context.args.outputDir ??
                path.join(context.args.dir, "generated"),
            config: path.isAbsolute(context.args.config)
                ? context.args.config
                : path.resolve(cwd(), context.args.config),
